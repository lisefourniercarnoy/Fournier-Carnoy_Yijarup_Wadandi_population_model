...
# Data:    ?
# Task:    Create a simulated fishing effort with WADANDI MANAGEMENT (shore effort only)
...
file_carpark          <- "data/input_data/wadandi_carparks.shp"
file_shore_effort <- "data/output_data/04B_shore_rec_effort.rds"
...
ggplot(water_long[water_long$Year %in% c(2020),]) +
  ...
  labs(title = "Catchability over time (S01_wadandi_management)", 
       fill = "log(Catchability)") +
  theme_minimal()
ggsave("plots/checking_plots_during_setup/S01_shore_rec_catchability.png", plot = last_plot())

# Save for future use.
saveRDS(spatial_q, file = paste0("data/output_data/S01_shore_rec_spatial_q_NTZ.rds"))
...
ggplot(water_catch_long %>% filter(access_point %in% unique(water_catch_long$access_point)[51:52])) + 
  ...
  labs(title = "(S01_wadandi_management) catchability of each cell, by shore access distance and cell size", fill = "log(Catchability)")
ggsave("plots/checking_plots_during_setup/S01_shore_rec_catchability_surface.png", plot = last_plot())
...
shore_effort_df <- readRDS("data/output_data/04B_shore_rec_effort.rds") # this is taken directly from the 04_B because i don't think it's different
...
ggsave("plots/checking_plots_during_setup/S01_shore_rec_effort_over_time_by_carpark.png", plot = last_plot())
...
built_CPs <- CP %>% filter(year_start < current_year | (year_start == current_year & month_strt <= current_month)); st_crs(built_CPs) <- 4326 # filter CPs that are already built
...
# Months
burn_in_effort <- readRDS("data/output_data/04B_shore_prop_month_ave.rds")[, 2] * effort
...
saveRDS(s_burn_in_fishing_full, file = "data/output_data/S01_shore_rec_burn_in_fishing.rds")
