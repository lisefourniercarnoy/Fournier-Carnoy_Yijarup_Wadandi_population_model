...
Rcpp::List pseudo_effort_function(
  ...
  for (int i = 0; i < n_fleets; i++) { // for each fleet...
   ...
   // obtain previous month's catch, effort, and calculate CPUE
    ...
    // TESST
    // std::fill(past_month_cpue.begin(), past_month_cpue.end(), 1);
    
    // add uncertainty 
    // we want to add uncertainty in the expected catch. we want the average recruitment to be what last month's catch was (uncertainty averaging 1), but some months above, some months below.
    ...
    //double sigma = 0.5;
    //double uncertainty = exp(R::rnorm(0, sigma) - ((sigma * sigma) / 2));
