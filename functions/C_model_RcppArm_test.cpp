...
Rcpp::List mortalityfunc_cpp(const int AGE, 
                             ...
                             arma::cube Selectivity_b_rec, 
                             arma::cube Selectivity_s_rec, 
                             
                             arma::cube YearlyTotal,
                             
                             arma::cube Effort_com, // input 3 sources of effort instead of Charlotte's 1
                             arma::cube Effort_b_rec,
                             arma::cube Effort_s_rec) {
  // declare object types
  int Cell_rw;
  arma::vec tot_survived(MaxCell); // number of fish that survived in each cell
  arma::vec tot_died(MaxCell); // number of fish that died in each cell
  
  arma::vec tot_caught(MaxCell); // number of fish that were caught in each cell
  arma::vec caught_com(MaxCell);
  arma::vec caught_b_rec(MaxCell);
  arma::vec caught_s_rec(MaxCell);
  
  arma::vec f_com(MaxCell); // declare the 3 fishing mortalities objects
  arma::vec f_b_rec(MaxCell);
  arma::vec f_s_rec(MaxCell);
  ...
  for(Cell_rw=0; Cell_rw<MaxCell; Cell_rw++){
    ...
    // below modified to add sources of fishing. (SoF)
    f_com(Cell_rw)        = Effort_com(Cell_rw, MONTH, YEAR) * Selectivity_com(AGE,MONTH,YEAR); // was Charlotte's finite_f
    f_b_rec(Cell_rw)      = Effort_b_rec(Cell_rw, MONTH, YEAR) * Selectivity_b_rec(AGE,MONTH,YEAR);
    f_s_rec(Cell_rw)      = Effort_s_rec(Cell_rw, MONTH, YEAR) * Selectivity_s_rec(AGE,MONTH,YEAR);

    Z(Cell_rw) = f_com(Cell_rw) + f_b_rec(Cell_rw) + f_s_rec(Cell_rw) + NatMort/12; // added SoF here

    tot_survived(Cell_rw) = YearlyTotal(Cell_rw,MONTH,AGE) * exp(-Z(Cell_rw)); //  previously : ((f_com(Cell_rw) + f_b_rec(Cell_rw) + f_s_rec(Cell_rw)) * exp(-NatMort/12));
    tot_died(Cell_rw)     = YearlyTotal(Cell_rw,MONTH,AGE) - tot_survived(Cell_rw);
    tot_caught(Cell_rw)   = YearlyTotal(Cell_rw,MONTH,AGE) * (1-(exp(-(f_com(Cell_rw) * f_b_rec(Cell_rw) * f_s_rec(Cell_rw)))));
 
    caught_com(Cell_rw)   = (f_com(Cell_rw)   / Z(Cell_rw)) * tot_caught(Cell_rw); // this calculates each fishing source's catch, as a proportion of total catch.
    caught_b_rec(Cell_rw) = (f_b_rec(Cell_rw) / Z(Cell_rw)) * tot_caught(Cell_rw);
    caught_s_rec(Cell_rw) = (f_s_rec(Cell_rw) / Z(Cell_rw)) * tot_caught(Cell_rw);
    
    YPR(Cell_rw) = tot_survived(Cell_rw)*((f_com(Cell_rw) + f_b_rec(Cell_rw) + f_s_rec(Cell_rw))/Z(Cell_rw)) * (1-(exp(-Z(Cell_rw)))) * Weight(AGE, MONTH); // really unsure about the addition of the 3 f here...
    
    Baranov(Cell_rw) = tot_survived(Cell_rw) * ((f_com(Cell_rw) + f_b_rec(Cell_rw) + f_s_rec(Cell_rw))/Z(Cell_rw)) * (1-(exp(-Z(Cell_rw))));
  ...
  return Rcpp::List::create(Rcpp::Named("tot_survived") = tot_survived,
                            ...
                            Rcpp::Named("total_catch") = tot_caught,
                            Rcpp::Named("commercial_catch") = caught_com,
                            Rcpp::Named("boat_rec_catch") = caught_b_rec,
                            Rcpp::Named("shore_rec_catch") = caught_s_rec,
                            
                            Rcpp::Named("f_com") = f_com,
                            Rcpp::Named("f_b_rec") = f_b_rec,
                            Rcpp::Named("f_s_rec") = f_s_rec,
...
Rcpp::List RunModelfunc_cpp(const int YEAR, 
  ...
  Rcpp::NumericVector f_com(MaxCell);
  Rcpp::NumericVector f_b_rec(MaxCell);
  Rcpp::NumericVector f_s_rec(MaxCell);
  ...
  arma::cube tot_month_catch(MaxCell, 12, MaxAge);
  arma::cube com_month_catch(MaxCell, 12, MaxAge);
  arma::cube b_rec_month_catch(MaxCell, 12, MaxAge);
  arma::cube s_rec_month_catch(MaxCell, 12, MaxAge); // for now doing catch numbers. can make catch weight by fleet later.
  ...
  for (MONTH=0; MONTH<12; MONTH++) { 
    ...
    for (AGE=0; AGE<MaxAge; AGE++) {
      ...
      if (MONTH==11) {
        if (AGE<(MaxAge-1)) {
          ...
          Res = mortalityfunc_cpp(AGE, MaxCell, MONTH, YEAR, NatMort, Weight, Selectivity_com, Selectivity_b_rec, Selectivity_s_rec, YearlyTotal, Effort_com, Effort_b_rec, Effort_s_rec);
          
          tot_survived = Res["tot_survived"];
          tot_died = Res["tot_died"];
          caught = Res["age_Baranov"];
          caught_weight = Res["catch_weight"];
          YPR_month = Res["YPR"]; // all 5 confirmed outputs from the mortality function
          
          tot_caught     = Rcpp::as<arma::vec>(Res["total_catch"]);
          caught_com     = Rcpp::as<arma::vec>(Res["commercial_catch"]);
          caught_b_rec   = Rcpp::as<arma::vec>(Res["boat_rec_catch"]);
          caught_s_rec   = Rcpp::as<arma::vec>(Res["shore_rec_catch"]);
      ...
      for (Cell_rw=0; Cell_rw<MaxCell; Cell_rw++) {
        tot_month_catch(Cell_rw,MONTH,AGE) = tot_caught(Cell_rw);
        com_month_catch(Cell_rw,MONTH,AGE) = caught_com(Cell_rw); // adding other outputs for fleet-specific catches
        b_rec_month_catch(Cell_rw,MONTH,AGE) = caught_b_rec(Cell_rw);
        s_rec_month_catch(Cell_rw,MONTH,AGE) = caught_s_rec(Cell_rw);
    ...
    } // this inner loop calculates extra metrics (inc. fishing and other mortality metrics)
  ...
  return Rcpp::List::create(Rcpp::Named("YearlyTotal") = YearlyTotal, // outputs
                            ...
                            Rcpp::Named("tot_month_catch") = tot_month_catch,
                            Rcpp::Named("com_month_catch") = com_month_catch,
                            Rcpp::Named("b_rec_month_catch") = b_rec_month_catch,
                            Rcpp::Named("s_rec_month_catch") = s_rec_month_catch,
