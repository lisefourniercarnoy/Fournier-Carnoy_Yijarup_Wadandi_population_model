...
total.pop.format.full <- function(pop.file.list, scenario.names, nsim, nyears, startyear, maxage, mat, kg){
  ...
  total_pop <- NULL
  mod_year <- seq(1946, 2024, 1) # the list of years you're modelling. im doing from 1946 for now, the burn-in is 79 years, need to fix that
  
  for(i in 1:length(pop.file.list)){ # for every scenario you're simulating
    temp <- pop.file.list[[i]]
    
    median.ages <- NULL
    
    for(year in 1:length(mod_year)){ # and for every year within that simulation
      
      age <- temp[, year,] # obtain the age structure of the population...
      
      age.median <- age %>% 
        as.data.frame()
      
      age.median <- as.data.frame(age.median[ , ] * mat[,12]) # ...and calculate how many are mature
      age.median <- as.data.frame(age.median[ , ] * kg[,12]) # and also the biomass of that mature group
      
      age.median <- age.median[, 1:nsim] %>% # then keep the mature biomass from all the simulations
        as.data.frame() %>% 
        mutate(Year = mod_year[year]) #%>% 
        #rename(MatBio = ".") # cant rename multiple columns with 'matbio'
