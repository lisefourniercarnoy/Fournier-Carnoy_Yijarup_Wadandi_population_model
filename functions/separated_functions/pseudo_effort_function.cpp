...
Rcpp::List pseudo_effort_function(
  ...
  for (int i = 0; i < n_fleets; i++) { // for each fleet...
   ...
   // obtain previous month's catch, effort, and calculate CPUE
    ...
    // add uncertainty 
    double uncertainty = pow(R::rnorm(0, 0.3), 2);
    arma::vec temp = past_month_cpue * std::sqrt(uncertainty); // transform into *expected catch* by adding uncertainty - store in a temporary object
