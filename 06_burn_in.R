...
BURN_IN_pop_monthly <- list() # list, year -> month -> (cell x length x age)
...
for (YEAR in 0:(max_year-1)){ # max_year-1 because it starts at 0.
  ...
  BURN_IN_pop_monthly[[YEAR+1]]  <- ModelOutput$master_current_pop # list of 12, each (cell x length x age)
...
age_df <- lapply(ages_to_plot, function(a) {
  cell_totals <- apply(BURN_IN_pop[[year_to_plot]][,, a], 1, sum)
...
for (YEAR in 1:n_years_done) {
  ...
  png(
    filename = file.path(
      ...
      sprintf("recons_year_%04d.png", YEAR)
    ),
    width = 1200, height = 500, res = 120
  )
