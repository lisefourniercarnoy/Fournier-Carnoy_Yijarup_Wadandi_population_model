...
ggplot(water_dist_long %>% dplyr::filter(AP %in% unique(water_dist_long$AP)[1:5])) + 
...
for (YEAR in 1:n_years_tot) {
  for (MONTH in 1:12) {
    for (ACCESS in 1:nrow(AP)) {
      #attractivity[, ACCESS, MONTH, YEAR] <- build_year[ACCESS, YEAR] * (1 / (as.numeric(access_dist[,ACCESS]) + 1))
      
      attractivity[, ACCESS, MONTH, YEAR] <- 
        build_year[ACCESS, YEAR] * (
        -0.204 * (2 * as.numeric(access_dist[, ACCESS])) + # travel cost, with round-trip distance
...
mapview::mapview(water %>% 
                   ...
                   mutate(test = attractivity[, test_access, test_month, test_year]),
                 zcol = "test") +
  mapview::mapview(AP[test_access,])

#convert to a format that C++ deals with better
...
fishable_area <- water_area # cell x month x year
...
## 5.a. enter the overall effort values (boat days) ---------------------------
...
## 5.b. split yearly fishing effort by month ----------------------------------
...
## 5.c. distribute monthly effort into access points --------------------------
...
# 6. create a list to use in the C++ function ---------------------------------

# little details to include in the final list
cell_area <- water$cell_area / 1000000 # just cell area
...
## 7. SETTING UP EFFORT FOR BURN IN -------------------------------------------
